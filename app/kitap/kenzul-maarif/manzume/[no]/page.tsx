...
export function generateStaticParams() {
  return getKenzulEntries()
    .filter((e) => e.type === "manzume")
    .map((e) => ({ no: String(e.no) }));
}

export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  const { no } = await params;
  return { title: `Manzume ${no} — Kenz'il-Maarif İlmihali` };
}

export default async function KenzulManzumePage({ params }: { params: Params }) {
  const { no } = await params;
  const n = /^\d+$/.test(no) ? parseInt(no, 10) : NaN;
  return <KenzulReader type="manzume" no={Number.isNaN(n) ? -1 : n} />;
}
